...
birdsay <- function(
  ...
  if (is.null(say)) {
    ...
    if (source == "statquotes" && has_statquotes) {
        say <- paste(
          capture.output(statquotes:::print.statquote(statquotes::statquote(), width = width - bird_width)),
          collapse = "\n"
        )
    } else if (source == "fortunes" && has_fortunes) {
      say <- paste(
        capture.output(fortunes:::print.fortune(fortunes::fortune(), width = width - bird_width)),
        collapse = "\n"
      )
    } else {
